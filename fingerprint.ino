...
void setup() {
  ...
  CardService::writeJsonToSD("user_record.json", user1);
  delay(2000);

  // User *user2 = new User(2, "zin bo khine", "password", 5, "MC", "ext 5");
  // CardService::writeJsonToSD("user_record.json", user2);

  // User *user3 = new User(3, "may su lei yee htay", "password", 5, "MC", "1");
  // CardService::writeJsonToSD("user_record.json", user3);

  // User *user4 = new User(4, "nilar tun", "password", 5, "MC", "ext 1");
  // CardService::writeJsonToSD("user_record.json", user4);

  // User *user5 = new User(5, "kyi khant", "password", 5, "MC", "ext 4");
  // CardService::writeJsonToSD("user_record.json", user5);

  delay(2000);
  CardService::readJsonFromSD("user_record.json");
